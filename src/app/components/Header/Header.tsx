...
const Header = ({ showLargeLogo = false, cartLength = 0, lang = 'en', currency = 'eur', invert = false }: HeaderProps) => {
    ...
    return (
        <>  
            { showLargeLogo && (
            <svg width="100%" height="100%" viewBox="0 0 595 80" fill="none" className={`${invert ? 'text-white' : 'text-black'} overflow-visible flex w-full h-[var(--logoHeight)] pt-[--topSpacing] px-[--sideSpacing] relative top-0 z-10 mb-[-.25rem]`} xmlns="http://www.w3.org/2000/svg">
            ...
            <header className={`${showLargeLogo ? 'mb-[calc(0px_-_var(--logoHeight)_-_var(--headerHeight)_+_0.25rem)] sticky' : 'fixed inset-x-0' } ${invert ? 'text-white' : 'text-black'} ${styles.header} top-0 w-full grid grid-flow-row grid-cols-[minmax(min-content,_1fr)_auto_minmax(min-content,_1fr)] md:grid-cols-2 gap-x-8 py-[--topSpacing] px-[--sideSpacing]  z-50 ${isOpen ? styles.open : ''}`}>
                ...
                <nav className={`flex gap-x-8 flex-wrap self-center justify-center font-medium md:absolute md:z-0 md:bg-white md:top-0 md:left-0 md:right-0 md:flex-col md:items-center md:text-center md:px-side-xs md:pt-[calc(var(--headerHeight)_+_0.25rem_+_3rem)] md:pb-lg md:gap-y-3xs ${isVisible ? '' : styles.close} `}>
