...
<script>
  ...
  const calc = (val, elRect) => {
    elRect = typeof elRect === 'number' && !isNaN(elRect) ? elRect : lenis.dimensions.height;
    let pixelVal = val.replace(/(\d+)(vw|vh|%|rem|px)?/g, (match, number, unit) => {
      number = parseFloat(number);
      switch (unit) {
        case 'vw': return number * lenis.dimensions.width / 100;
        case 'vh': return number * lenis.dimensions.height / 100;
        case '%': return number * elRect / 100;
        case 'rem': return number * parseFloat(getComputedStyle(document.documentElement).fontSize);
        case 'px':
  ...
  const observeElement = (el, {scrub = false, offset = {}, name = '', scope, ease = (x) => { return x }} = []) => {
    ...
    const data = {
      ...
      name: name && `${name}-`,
      rect: elRect,
      offset: { top:oCalc('top'),right:oCalc('right'),bottom:oCalc('bottom'),left:oCalc('left'),},
      progress: {x:0,y:0,},
      isVisible: false,
  ...
  const updateScrollProgress = (data, el) => {
    ...
    [p.x, p.y] = end.map((e, i) => Math.min(Math.max((e - dist[i]) / -dist[i], 0), 1));
    const isBefore = [(p.x === 0), (p.y === 0)];
    const hasPassed = [(p.x === 1), (p.y === 1)];
    const isInView = [(!isBefore[0] && !hasPassed[0]), (!isBefore[1] && !hasPassed[1])];
  ...
  const observeElements = document.querySelectorAll('[data-observe-inview="true"], [data-observe-inview-scrub="true"]');
  ...
  const updateScrollEls = (run) => {
      scrollEls.forEach((data, el) => (data.isVisible || run) && updateScrollProgress(data, el));
  };
...
const setHeaderObserver = () => {
    ...
    },  { rootMargin: `-${hcRect.height}px 0px 0px 0px`, threshold: 0, root: document });

    observeHeader.observe(hw);


    observeHeaderStyle = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          const styleAttributes = entry.target.getAttribute('data-observe-header-style').split(';').filter(style => style);
    ...
    }, { rootMargin: `0px 0px -${lenis.dimensions.height - hcRect.height}px 0px`, threshold: 0, root: document });

    document.querySelectorAll('[data-observe-header-style]').forEach(element => observeHeaderStyle.observe(element));
};
...
new ResizeObserver(entries => {
  ...
  /* Single product page gallery side scroller */
  
  const pdpImageScroll = () => {
    const wrapper = document.querySelector('.pdp-images-list');
    if(!wrapper) return;
    const lenisPdp = new Lenis({
      wrapper: wrapper,
    ...
    lenisPdp.on('scroll', (e) => {
      const isEnd = (lenisPdp.dimensions.scrollWidth- lenisPdp.dimensions.width - lenisPdp.targetScroll <= 0);
      if (isEnd && lenisPdp.direction === 1) wrapper.style.setProperty('pointer-events', 'none');
    })
    
    lenis.on('scroll', (e) => {
      if (lenis.targetScroll === 0) wrapper.style.setProperty('pointer-events', 'auto');
    })
    
    function pdpRaf(time) {
      lenisPdp.raf(time);
      requestAnimationFrame(pdpRaf);
    }
    requestAnimationFrame(pdpRaf);
  }
  ...
  document.querySelector('.cta-btn')?.addEventListener('click', ()=> { lenis.scrollTo('#pdpsettings'); })
